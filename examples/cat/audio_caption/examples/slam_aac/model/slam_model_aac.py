...
class slam_model_aac(slam_model):
    ...
    def forward(self,
        ...
        if audio_mel is not None or audio is not None:
            if self.model_config.encoder_name == "ast":
                encoder_outs = self.encoder.forward(audio_mel.squeeze(dim=1), return_features=True)
            if self.model_config.encoder_name == "audiomae":
                encoder_outs = self.encoder.forward_features(audio_mel, feature_only=True)
