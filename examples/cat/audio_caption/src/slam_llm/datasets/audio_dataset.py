...
class AudioDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.model_name == "beats":
        ...
        elif self.model_name == "audiomae":
            args_dataset = 'audioset'
            ...
            audio_conf_val = {'num_mel_bins': 128, 
                    ...
                    'dataset': 'audioset',
                    'mode':'val',
                    'mean':norm_stats[args_dataset][0],
                    'std':norm_stats[args_dataset][1],
                    'noise':False,
                    'multilabel':multilabel_dataset[args_dataset],
            }  
            waveform = audio_raw - audio_raw.mean()
            fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, use_energy=False,window_type='hanning', num_mel_bins=audio_conf_val['num_mel_bins'], dither=0.0, frame_shift=10)
            target_length = audio_conf_val['target_length']
            ...
            if p > 0:
                ...
                audio_mel = fbank[0:target_length, :]
        elif self.model_name == "ast":
            fbank = torchaudio.compliance.kaldi.fbank(
            audio_raw, htk_compat=True, sample_frequency=16000, use_energy=False,
        ...
        if self.model_name == "beats":
            ...
            audio_length = audio_mel.shape[0] // 2 + 1 
        elif self.model_name == "ast":
            audio_length = audio_mel.shape[0] // 2 + 1 
