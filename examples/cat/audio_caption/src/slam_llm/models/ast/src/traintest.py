...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels) in enumerate(train_loader):
            ...
            with autocast():
                audio_output, features = audio_model(audio_input, return_disp_feature_layer=args.disp_loss_layer, return_disp_feature_cls=args.disp_loss_cls)
                ...
                d_loss = args.disp_loss_weight * disp_loss(features)
                if isinstance(loss_fn, torch.nn.CrossEntropyLoss):
                    c_loss = loss_fn(audio_output, torch.argmax(labels.long(), axis=1))
                else:
                    c_loss = loss_fn(audio_output, labels)
                loss = c_loss + d_loss
            ...
            c_loss_meter.update(c_loss.item(), B)
            d_loss_meter.update(d_loss.item(), B)
            ...
            if print_step and global_step != 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Cls Loss {c_loss_meter.avg:.4f}\t'
                  'Disp Loss {d_loss_meter.avg:.4f}\t'.format(
                   epoch, i, len(train_loader), per_sample_time=per_sample_time, per_sample_data_time=per_sample_data_time,
                      per_sample_dnn_time=per_sample_dnn_time, loss_meter=loss_meter, c_loss_meter=c_loss_meter, d_loss_meter=d_loss_meter), flush=True)
