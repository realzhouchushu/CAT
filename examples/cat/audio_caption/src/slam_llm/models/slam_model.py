...
def setup_encoder(train_config, model_config, **kwargs):
    ...
    if len(encoder_list) == 1:
        ...
        if encoder_name == "ast":
            from slam_llm.models.encoder import ASTEncoder
            encoder = ASTEncoder.load(model_config)
        if encoder_name == "audiomae":
            from slam_llm.models.encoder import AudioMAEEncoder
            encoder = AudioMAEEncoder.load(model_config)
