...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        # 1. 定义上下文管理器列表
        if train_config.enable_fsdp or train_config.enable_ddp:
            # 列表包含已命名的 memtrace 实例和 Join 实例
            context_managers = [memtrace, Join([model])]
        else:
            # 列表只包含已命名的 memtrace 实例
            context_managers = [memtrace]
        ...
        with ExitStack() as stack:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (epoch * total_length + step + 1 if train_config.batching_strategy != "dynamic" else step + 1) % train_config.validation_interval == 0:
                    ...
                    if train_config.save_model and (eval_epoch_loss < best_val_loss or train_config.run_validation == False):
