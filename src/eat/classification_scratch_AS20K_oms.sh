...
model_model_path=/opt/gpfs/home/chushu/exp/eat/pre_8_AS2M/default_bsz16/checkpoint_last.pt

optimizer_groups_default_lr_float=${2:-0.00002}
SAVE_DIR_ROOT=/opt/gpfs/home/chushu/exp/eat/cfc_8_AS20k

# # 从 model_model_path 提取父目录名与文件名
parent_dir="$(basename -- "$(dirname -- "$model_model_path")")"
ckpt_name="$(basename -- "$model_model_path")"
# 组合保存目录并确保存在
checkpoint_save_dir="${SAVE_DIR_ROOT%/}/${parent_dir}/${optimizer_groups_default_lr_float}"
...
python fairseq_cli/hydra_train.py -m \
    ...
    # CLS_TOKEN CLAP_TYPE2 num_workers: 6 data_buffer_size: 10